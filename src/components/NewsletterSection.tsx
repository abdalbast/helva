...
const NewsletterSection = () => {
  ...
  return (
    <section ref={sectionRef} className="col-span-12 pt-14 lg:pt-16">
      <div className={`max-w-3xl transition-all duration-1000 ease-out ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
        <div className="border-t border-border/30 pt-10">
          <span className="font-mono text-[0.65rem] uppercase tracking-[0.3em] text-muted-foreground mb-5 block">{t('newsletter.stayConnected')}</span>
          <h2 className="font-display font-bold text-3xl lg:text-4xl tracking-tight text-foreground mb-4">{t('newsletter.joinCircle')}</h2>
          <p className="text-foreground/70 text-base lg:text-lg leading-relaxed mb-8 max-w-xl">{t('newsletter.subtitle')}</p>
        {!isSubmitted ? (
          <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-4 max-w-2xl">
            <div className="relative flex-1">
              <label htmlFor="newsletter-email" className="sr-only">Email</label>
              <input id="newsletter-email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder={t('home.emailPlaceholder')} required disabled={isLoading} className="w-full bg-transparent border-b border-border/40 py-3 px-0 text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:border-primary transition-colors duration-300 font-light text-base disabled:opacity-50" />
            ...
            <button type="submit" disabled={isLoading} className="bg-foreground text-background px-8 py-3 font-mono tracking-[0.18em] uppercase text-xs transition-opacity duration-300 hover:opacity-85 focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 focus:ring-offset-background disabled:opacity-50 disabled:cursor-not-allowed">{isLoading ? t('newsletter.joining') : t('newsletter.join')}</button>
          </form>
        ) : (
