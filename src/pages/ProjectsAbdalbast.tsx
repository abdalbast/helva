      ...
      <section className="col-span-12 mb-10 space-y-4">
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {CATEGORIES.map((c) => <FilterPill key={c} label={c} active={activeCategory === c} onClick={() => setActiveCategory(activeCategory === c ? null : c)} />)}
          </div>
        </div>
        <div>
          <span className="block font-mono text-[0.6rem] uppercase tracking-[0.2em] text-muted-foreground/60 mb-2">Status</span>
          <div className="flex flex-wrap gap-2">
            {STATUSES.map((s) => <FilterPill key={s} label={s} active={activeStatus === s} onClick={() => setActiveStatus(activeStatus === s ? null : s)} />)}
          </div>
        </div>
      </section>
