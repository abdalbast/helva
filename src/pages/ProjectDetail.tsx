      ...
      <>
        ...
        <main className="min-h-screen grid grid-cols-12 p-5 lg:p-10 gap-5">
          ...
          <section className="col-span-12 lg:col-span-6 lg:col-start-4 py-24 text-center">
            <h1 className="font-display font-extrabold text-4xl text-primary tracking-tighter mb-4">
              Project Not Found
            </h1>
    ...
    <>
      ...
      <main className="min-h-screen grid grid-cols-12 p-5 lg:p-10 gap-5">
        ...
        <section className="col-span-12 lg:col-span-8 lg:col-start-3 py-16 lg:py-24">
          <Link
            to="/projects"
            className="font-mono text-xs uppercase tracking-[0.15em] text-muted-foreground hover:text-primary transition-colors mb-8 inline-block animate-reveal"
          >
            ← Projects
          </Link>
          ...
          <h1 className="animate-reveal stagger-2 font-display font-extrabold text-[clamp(2.5rem,6vw,5rem)] leading-[0.95] text-primary tracking-tighter mb-6">
            {project.title}
          </h1>
          <p className="animate-reveal stagger-3 text-xl lg:text-2xl text-foreground/80 max-w-2xl leading-relaxed font-light">
            {project.heroTagline}
          </p>
        </section>
        ...
        <section className="col-span-12 lg:col-span-8 lg:col-start-3 py-8">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {project.metrics.map((metric, idx) => (
              <div
                key={metric.label}
                className={`animate-reveal stagger-${(idx % 4) + 1} p-6 border border-border/30 bg-card/20`}
              >
                <span className="block font-display font-extrabold text-2xl lg:text-3xl text-primary tracking-tight mb-1">
                  {metric.value}
                </span>
        ...
        {[
          ...
          <section
            key={section.label}
            className={`col-span-12 lg:col-span-8 lg:col-start-3 py-8 ${idx < 2 ? 'border-b border-border/20' : ''}`}
          >
        ...
        {project.testimonial && (
          <section className="col-span-12 lg:col-span-8 lg:col-start-3 py-12">
            <div className="border-l-2 border-primary/30 pl-8">
              <blockquote className="text-xl lg:text-2xl text-foreground/80 font-light leading-relaxed italic mb-6">
                "{project.testimonial.quote}"
              </blockquote>
              <div>
                <span className="block font-display font-bold text-foreground text-sm">
        ...
        <section className="col-span-12 lg:col-span-8 lg:col-start-3 py-8 border-t border-border/20">
          <div className="flex flex-wrap gap-2">
            {project.tags.map((tag) => (
              <span
                key={tag}
                className="font-mono text-[0.65rem] uppercase tracking-[0.15em] text-muted-foreground px-3 py-1.5 border border-border/30"
              >
                {tag}
              </span>
            ))}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {otherProjects.map((p) => (
                <Link
                  key={p.slug}
                  to={`/projects/${p.slug}`}
                  className="group p-8 bg-card/30 border border-border/30 hover:border-primary/30 transition-all duration-500 hover:bg-card/50"
                >
